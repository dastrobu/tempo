...
func TestDiscardCountReplicationFactor(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			discardedTraces := discardedTraces(numSuccessByTraceIndex, lastErrorReasonByTraceIndex, traceByID, tc.replicationFactor)
			liveTraceDiscardedCount, traceTooLongDiscardedCount, _ := countDiscardedSpans(discardedTraces)
...
func TestIngesterPushBytes(t *testing.T) {
	...
	discardedTraces := discardedTraces(numSuccessByTraceIndex, lastErrorReasonByTraceIndex, traces, 3)
	maxLiveDiscardedCount, traceTooLargeDiscardedCount, _ := countDiscardedSpans(discardedTraces)
